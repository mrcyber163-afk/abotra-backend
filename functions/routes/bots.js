...
// ============================================================
// 9. PROCESS DAILY PROFIT (Scheduled - Admin only or cron)
// ============================================================
router.post('/process-daily', verifyToken, async (req, res) => {
    try {
        ...
        const snapshot = await db.ref(`userRobots/${userId}`)
            .orderByChild('status')
            .equalTo('active')
            .once('value');
        ...
        snapshot.forEach(async (child) => {
            ...
            if (now >= nextProfitTime) {
                ...
                if (isLastDay) {
                    ...
                    await db.ref(`userRobots/${userId}/${robotId}`).update({
                        status: 'completed',
                        completedAt: now,
                        investedAmount: 0,
                        amount: 0
                    ...
                    // Add platform profit
                    const platformRef = db.ref('platformProfit');
                    await platformRef.transaction((current) => {
                        return (current || 0) + hiddenFee + finalFee;
                    });
