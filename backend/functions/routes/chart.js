// ============================================================
// CHART - REST API Version (No Admin SDK)
// ============================================================
...
router.get('/candles', verifyToken, async (req, res) => {
    try {
        ...
        res.json({ success: true, symbol: cleanSymbol, interval, candles });
    } catch (error) {
...
router.post('/settings', verifyToken, async (req, res) => {
    try {
        ...
        const existing = await restGet(`chartSettings/${userId}`) || {};
        await restPut(`chartSettings/${userId}`, { ...existing, ...settings });
...
router.get('/settings', verifyToken, async (req, res) => {
    try {
        const userId = req.user.uid;
        const settings = await restGet(`chartSettings/${userId}`) || {};
        res.json({
            success: true,
            settings: {
